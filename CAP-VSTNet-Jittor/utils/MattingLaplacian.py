...
def laplacian_loss_grad(image, M):
    ...
    for i in range(channel):
        grad = jt.nn.bmm(laplacian_m.unsqueeze(0), img[i, :, :].reshape(-1, 1).unsqueeze(0))/(height*width)
        loss += jt.nn.bmm(img[i, :, :].reshape(1, -1).unsqueeze(0), grad.unsqueeze(0))
        grads.append(grad.reshape((height, width)))
    gradient = jt.stack(grads, dim=0)
