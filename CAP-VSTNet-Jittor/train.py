...
# Optimizer
optimizer = jt.optim.Adam(RevNetwork.parameters(), lr=args.lr)
...
# Resume
if args.resume:
    state_dict = jt.load(os.path.join(checkpoint_directory, "last.pt"))
...
while current_iter < total_iterations:
    ...
    if args.lap_weight > 0:
        for M in images_a['laplacian_m']:
            indices = jt.transform.ToTensor(np.vstack((M.row, M.col))).long()
            values = jt.transform.ToTensor(M.data)
            shape = jt.Size(M.shape)
            laplacian_m = jt.transform.sparse_coo_tensor(indices, values, shape)            
    ...
    images_a, images_b = images_a['img'], images_b['img']
    ...
    if args.lap_weight > 0:
        ...
        loss_lap = jt.mean(jt.stack(lap_loss, dim=0))
    ...
    loss.backward()
    jt.nn.utils.clip_grad_norm_(RevNetwork.parameters(), 5)
    optimizer.step()
    ...
    if (current_iter + 1) % 10 == 0:
        ...
        if (current_iter + 1) % args.image_save_iter == 0:
            with jt.no_grad():
                index = jt.randint(low=0, high=len(train_loader_a.dataset), size=[args.display_size])
                train_display_images_a = jt.stack([train_loader_a.dataset[i]['img'] for i in index])
                index = jt.randint(low=0, high=len(train_loader_b.dataset), size=[args.display_size])
                train_display_images_b = jt.stack([train_loader_b.dataset[i]['img'] for i in index])
                train_image_outputs = RevNetwork.sample(cwct, train_display_images_a, train_display_images_b)
        ...
        if (current_iter + 1) % args.image_display_iter == 0:
            with jt.no_grad():
                ...
                index = jt.randint(low=0, high=len(train_loader_b.dataset), size=[args.display_size])
                train_display_images_b = jt.stack([train_loader_b.dataset[i]['img'] for i in index])
                image_outputs = RevNetwork.sample(cwct, train_display_images_a, train_display_images_b)
        ...
        if (current_iter + 1) % args.model_save_interval == 0:
            ckpoint_file = os.path.join(checkpoint_directory, f'batch_{batch_size}_epoch_{current_iter + 1}.pt')
            jt.save({'state_dict': RevNetwork.state_dict(), 'optimizer': optimizer.state_dict()}, ckpoint_file)

        if (current_iter + 1) == args.training_iterations:
            ckpoint_file = os.path.join(checkpoint_directory, 'model_image.pt')
            jt.save({'state_dict': RevNetwork.state_dict()}, ckpoint_file)
