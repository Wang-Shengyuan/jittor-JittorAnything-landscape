...
def merge(x1, x2):
    return jt.contrib.concat((x1, x2), dim=1)
...
class residual_block(nn.Module):
    ...
    def __init__(self, channel, stride=1, mult=4, kernel=3):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv(in_ch, channel // mult, kernel, stride=stride, padding=0, bias=True),
            nn.ReLU(),
            nn.ReflectionPad2d(pad),
            nn.Conv(channel // mult, channel // mult, kernel, padding=0, bias=True),
            nn.ReLU(),
            nn.ReflectionPad2d(pad),
            nn.Conv(channel // mult, channel, kernel, padding=0, bias=True)
        )
        self.init_layers()
...
class RevResNet(nn.Module):
    ...
    def execute(self, x, forward=True):
        if forward:
    ...
    def sample(self, transfer_module, x_c, x_s, device):
        ...
        x_cs = jt.contrib.concat(x_cs)
        x_c_cyc = jt.contrib.concat(x_c_cyc)
