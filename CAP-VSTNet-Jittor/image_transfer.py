...
state_dict = jt.load(args.ckpoint)
...
for each in filenames:
    ...
    transform = jt.transform.Resize((ori_csize[1], ori_csize[0]), interpolation=Image.BICUBIC)
    ...
    if args.auto_seg:
        ...
        seg_model = init_segmentor(config, checkpoint)
    ...
    content = jt.transform.ToTensor()(content).unsqueeze(0)
    style = jt.transform.ToTensor()(style).unsqueeze(0)
    ...
    grid = jittor_utils.make_grid(stylized.data, nrow=1, padding=0)
