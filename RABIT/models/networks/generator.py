...
class SEACEGenerator(BaseNetwork):
    ...
    def execute(self, warp_out=None):
        ...
        cmap1 = nn.interpolate(cmap.repeat(1, seg_feat1.shape[1], 1, 1), size=seg_feat1.size()[2:], mode='nearest')  # 8
        cmap2 = nn.interpolate(cmap.repeat(1, seg_feat2.shape[1], 1, 1), size=seg_feat2.size()[2:], mode='nearest')  # 16
        cmap3 = nn.interpolate(cmap.repeat(1, seg_feat3.shape[1], 1, 1), size=seg_feat3.size()[2:], mode='nearest')  # 32
        cmap4 = nn.interpolate(cmap.repeat(1, seg_feat4.shape[1], 1, 1), size=seg_feat4.size()[2:], mode='nearest')  #
        cmap5 = nn.interpolate(cmap.repeat(1, seg_feat5.shape[1], 1, 1), size=seg_feat5.size()[2:], mode='nearest')  # 8

        ref_feat1 = nn.interpolate(ref_feat1, size=seg_feat1.size()[2:])
        ref_feat2 = nn.interpolate(ref_feat2, size=seg_feat2.size()[2:])
        ref_feat3 = nn.interpolate(ref_feat3, size=seg_feat3.size()[2:])
        ref_feat4 = nn.interpolate(ref_feat4, size=seg_feat4.size()[2:])
        ref_feat5 = nn.interpolate(ref_feat5, size=seg_feat5.size()[2:])
        ...
        x = jt.cat((seg_feat5, ref_feat5), 1)
        x = nn.interpolate(x, size=(self.sh, self.sw))
        x = self.fc(x)
...
class DomainClassifier(BaseNetwork):
    def __init__(self, opt):
        ...
        self.feature = nn.Sequential(nn.Conv2d(4 * nf, 2 * nf, kw, stride=2, padding=pw),
                                ...
                                nn.LeakyReLU(0.2))  #32*8*8
        model = [nn.Linear(int(nf // 2) * 8 * 8, 100),
                nn.BatchNorm1d(100, affine=True),
                nn.ReLU()]
        self.domain_rela = opt.domain_rela
        if opt.domain_rela:
            model += [nn.Linear(100, 1)]
        else:
            model += [nn.Linear(100, 2),
                      nn.Softmax(dim=1),] # logsoftmax -> softmax
        self.classifier = nn.Sequential(*model)

    def execute(self, x):
        x = self.feature(x)
        x = self.classifier(x.view(x.shape[0], -1))
        if not self.domain_rela:
            x = jt.log(x)
        return x

class ReverseLayerF(jt.Function):

    @staticmethod
    def execute(ctx, x, alpha):
        ctx.alpha = alpha
