...
def ot_topk(logit, k, epsilon=5e-2, max_iter=10):
    # logit.shape=n, score
    logit = 1 - logit
    anchors = jt.FloatTensor([0, 1]).view([1, 1, 2])
    ...
    min_scores = jt.minimum(scores_).detach()
    ...
    nu = jt.FloatTensor([k/n, (n-k)/n]).view([1, 1, 2])
    nu = nu.repeat(bs, 1, 1)
...
def differentiable_topk_old(x, k, temperature=1.):
    *_, n, dim = x.shape
    ...
    topks = jt.cat(topk_tensors, dim=-1)
    return topks
...
def align_feature(R, dots, v):
    ...
    b_v_r = jt.einsum('buvk,bvtd->butkd', R, b_v)
