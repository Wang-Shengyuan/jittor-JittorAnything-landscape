...
def feature_normalize(feature_in):
    feature_in_norm = jt.norm(feature_in, 2, 1, keepdim=True) + sys.float_info.epsilon
    feature_in_norm = jt.div(feature_in, feature_in_norm)
    return feature_in_norm

def mean_normalize(feature, dim_mean=None):
    feature = feature - feature.mean(dim=dim_mean, keepdim=True)  # center the feature
    feature_norm = jt.norm(feature, 2, 1, keepdim=True) + sys.float_info.epsilon
    feature = jt.div(feature, feature_norm)
    return feature
...
def vgg_preprocess(tensor, vgg_normal_correct=False):
    ...
    tensor_bgr = jt.cat((tensor[:, 2:3, :, :], tensor[:, 1:2, :, :], tensor[:, 0:1, :, :]), dim=1)
    ...
    # print (1/0)
    tensor_bgr_ml = tensor_bgr - jt.Tensor([0.40760392, 0.45795686, 0.48501961]).type_as(tensor_bgr).view(1, 3, 1, 1)
