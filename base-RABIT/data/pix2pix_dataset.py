...
class Pix2pixDataset(BaseDataset):
    ...
    def semantic_position_encoding(self, label):
        ...
        label = nn.interpolate(label, size=(64, 64), mode='nearest')
        label = label.squeeze()

        h, w = label.shape
        coord = jt.zeros((3, h, w))
        x = jt.misc.arange(0, w, 1.0) / 255.0
        y = jt.misc.arange(0, h, 1.0) / 255.0
        xx, yy = jt.misc.meshgrid(x, y)

        ids = jt.misc.unique(label)
        for id in ids:
            mask = (label == id).float()
            xx_masked = xx * mask
            yy_masked = yy * mask
            ...
            rr_relative = jt.sqrt((jt.pow(xx_relative, 2) + jt.pow(yy_relative, 2)).float())
