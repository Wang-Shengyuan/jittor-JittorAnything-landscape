...
def vgg_preprocess(tensor, vgg_normal_correct=False):
    ...
    tensor_bgr_ml = tensor_bgr - jt.Var([0.40760392, 0.45795686, 0.48501961]).view(1, 3, 1, 1)
    tensor_rst = tensor_bgr_ml * 255
    # tensor_rst = tensor_bgr_ml
    return tensor_rst
...
def cal_feat_dist(feat_1, feat_2, label, use_cos):
    label_u = jt.unique(label)
    theta_value = 0
    for k in range(len(label_u)):
        num_1 = (label == label_u[k]).sum().item()
        mask = jt.zeros_like(label)
...
# TODO:考虑是否与现有地址适配
vgg.load_state_dict(jt.load('Jittor/base-RABIT/models/vgg19_conv.pth'))
...
for i in range(len(dataset)):
    ...
    for j in range(len(layers)):
        ...
        gen_img_feat = nn.interpolate(gen_img_features[j], size=[256, 256], mode='nearest').squeeze()
        img_feat = img_feat.view(img_feat.shape[0], -1)
        img_feat = jt.divide(img_feat, jt.norm(img_feat, p=2, dim=0, keepdim=True).expand_as(img_feat))
        gen_img_feat = gen_img_feat.view(gen_img_feat.shape[0], -1)
        gen_img_feat = jt.divide(gen_img_feat, jt.norm(gen_img_feat, p=2, dim=0, keepdim=True).expand_as(gen_img_feat))
