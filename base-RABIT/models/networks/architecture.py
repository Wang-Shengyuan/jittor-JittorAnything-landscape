...
class Attention(nn.Module):
    def __init__(self, ch, use_sn):
        ...
        self.gamma = nn.Parameter(jt.zeros(1), requires_grad=True)

    def execute(self, x, y=None):
        # Apply convs
        theta = self.theta(x)
        phi = nn.max_pool2d(self.phi(x), [2,2])
        g = nn.max_pool2d(self.g(x), [2,2])    
        # Perform reshapes
        theta = theta.view(-1, self. ch // 8, x.shape[2] * x.shape[3])
        phi = phi.view(-1, self. ch // 8, x.shape[2] * x.shape[3] // 4)
        g = g.view(-1, self. ch // 2, x.shape[2] * x.shape[3] // 4)
        # Matmul and softmax to get attention maps
        beta = nn.softmax(nn.bmm(theta.transpose(1, 2), phi), -1)
        # Attention map times g path
        o = self.o(nn.bmm(g, beta.transpose(1,2)).view(-1, self.ch // 2, x.shape[2], x.shape[3]))
...
class VGG19(nn.Module):
    def __init__(self, requires_grad=False):
        super().__init__()
        vgg_pretrained_features = jt_models.vgg19(pretrained=True).features
