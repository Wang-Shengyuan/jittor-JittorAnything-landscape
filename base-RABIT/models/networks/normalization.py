...
class SEACE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, PONO=False, use_apex=False, feat_nc=None, atten=False):
        ...
        # TODO:是否可行？
        self.coef = nn.Parameter(jt.array(0.), requires_grad=True)

    def execute(self, x, seg_map, ref_map, atten_map, conf_map):
        normalized = self.param_free_norm(x)
        b, _, h, w = x.size()

        seg_map = nn.interpolate(seg_map, size=(h, w), mode='nearest')
        ref_map = nn.interpolate(ref_map, size=(h, w), mode='nearest')
        conf_map = nn.interpolate(conf_map, size=(h, w), mode='nearest')
        ...
        if self.atten:
            if h <= 64:
                ...
                ref_aggr = nn.bmm(ref_feat.view(b, self.feat_nc, h*w), atten_map.permute(0, 2, 1))
                ref_aggr = ref_aggr.view(b, self.feat_nc, h, w)
            else:
                ref_map_aggr = nn.interpolate(ref_map, size=(64, 64), mode='nearest')
                ref_feat = self.ref_shared(ref_map_aggr)

                ref_aggr = nn.bmm(ref_feat.view(b, self.feat_nc, 64 * 64), atten_map.permute(0, 2, 1))
                ref_aggr = ref_aggr.view(b, self.feat_nc, 64, 64)
                ref_aggr = nn.interpolate(ref_aggr, size=(h, w), mode='nearest')
        ...
        feat = nn.interpolate(feat, size=(h, w), mode='nearest')
...
class SPADE(nn.Module):
    ...
    def execute(self, x, segmap, similarity_map=None):
        ...
        if similarity_map is not None:
            similarity_map = nn.interpolate(similarity_map, size=gamma.size()[2:], mode='nearest')
...
class SPADE_TwoPath(nn.Module):
    ...
    def execute(self, x, warpmap, segmap, similarity_map):
        ...
        # Part 2. produce scaling and bias conditioned on semantic map
        segmap = nn.interpolate(segmap, size=x.size()[2:], mode='nearest')
        warpmap = nn.interpolate(warpmap, size=x.size()[2:], mode='nearest')
        ...
        similarity_map = nn.interpolate(similarity_map, size=x.size()[2:], mode='nearest')
