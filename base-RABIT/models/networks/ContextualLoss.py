...
from jittor import transform as transform
import numpy as np
...
class ContextualLoss(nn.Module):
    ...
    def execute(self, X_features, Y_features, h=0.1, feature_centering=True):
        ...
        d_norm = d / (jt.min(d, dim=-1, keepdim=True)[0] + 1e-5)  # batch_size * feature_size^2 * feature_size^2

        # pairwise affinity
        w = jt.exp((1 - d_norm) / h)
        A_ij = w / jt.sum(w, dim=-1, keepdim=True)

        # contextual loss per sample
        CX = jt.mean(jt.max(A_ij, dim=1)[0], dim=-1)
        loss = -jt.log(CX)
...
class ContextualLoss_forward(nn.Module):
    ...
    def execute(self, X_features, Y_features, h=0.1, feature_centering=True):
        ...
        # normalized distance: dij_bar
        # d_norm = d
        d_norm = d / (jt.min(d, dim=-1, keepdim=True)[0] + 1e-3)  # batch_size * feature_size^2 * feature_size^2
...
class ChamferDistance_patch_loss(nn.Module):
    ...
    def execute(self, X_features, Y_features, patch_size=3, image_x=None, image_y=None, h=0.1, Y_features_in=None):
        ...
        if Y_features_in is not None:
            loss = jt.mean((X_features - Y_features_in.detach())**2)
            Y_features_in = Y_features_in.detach()
        else:
            loss = jt.mean((X_features - Y_features[:, :, NN_index].detach())**2)
