...
class NoVGGCorrespondence(BaseNetwork):
    ...
    def execute(self, ref_img, real_img, seg_map, ref_seg_map, detach_flag=False):
        ...
        if self.opt.maskmix:
            cont_features = self.layer(jt.concat((adp_feat_seg, seg), 1))
            ref_features = self.layer(jt.concat((adp_feat_img, ref_seg), 1))

            cont_features6 = self.layer6(jt.concat((adp_feat_seg6, seg6), 1))
            ref_features6 = self.layer6(jt.concat((adp_feat_img6, ref_seg6), 1))

        else:
            cont_features = self.layer(adp_feat_seg)
            ref_features = self.layer(adp_feat_img)

            cont_features6 = self.layer6(adp_feat_seg6)
            ref_features6 = self.layer6(adp_feat_img6)
        ...
        # Confidence map.
        conf_map = jt.max(R[:, :, :, 0], -1, keepdims=True)[0]
        # print('*****', (conf_map - conf_map.mean(dim=1, keepdim=True)).shape)
        conf_map = (conf_map - conf_map.mean(dim=1, keepdims=True)).view(batch_size, 1, feat_height // 2, feat_width // 2)
        ...
        if self.opt.warp_cycle_w > 0:
            ...
            if self.opt.warp_patch:
                ...
                warp_cycle = jt.matmul(f_div_C_v, y_).permute(0, 2, 1)
                ...
                if self.opt.two_cycle:
                    ...
                    warp_i2r = jt.matmul(f_div_C_v, real_img).permute(0, 2, 1)  #warp input to ref
    ...
    def addcoords(self, x):
        bs, _, h, w = x.shape
        xx_ones = jt.ones([bs, h, 1], dtype=x.dtype, device=x.device)
        xx_range = jt.arange(w, dtype=x.dtype, device=x.device).unsqueeze(0).repeat([bs, 1]).unsqueeze(1)
        xx_channel = jt.matmul(xx_ones, xx_range).unsqueeze(1)

        yy_ones = jt.ones([bs, 1, w], dtype=x.dtype, device=x.device)
        yy_range = jt.arange(h, dtype=x.dtype, device=x.device).unsqueeze(0).repeat([bs, 1]).unsqueeze(-1)
        yy_channel = jt.matmul(yy_range, yy_ones).unsqueeze(1)
