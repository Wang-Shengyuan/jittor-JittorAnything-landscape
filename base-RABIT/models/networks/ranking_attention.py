...
def ot_topk(logit, k, epsilon=5e-2, max_iter=10):
    # logit.shape=n, score
    logit = 1 - logit
    anchors = jt.Var([0, 1]).view([1, 1, 2])
    anchors = anchors.cuda()
    bs, n = logit.size()
    logit = logit.view([bs, n, 1])

    scores_ = logit.clone().detach()
    max_scores = jt.max(scores_).detach()
    scores_[scores_==float('-inf')] = float('inf')
    min_scores = jt.min(scores_).detach()
    ...
    C = (scores-anchors)**2
    C = C / (C.max().detach())
    mu = jt.ones([bs, n, 1], requires_grad=False)/n
    nu = jt.Var([k/n, (n-k)/n]).view([1, 1, 2])
    nu = nu.repeat(bs, 1, 1)
...
def differentiable_topk(logit, k):
    ...
    topk_tensors = jt.zeros_like(x).scatter_(-1, indices, values)
...
def differentiable_topk_old(x, k, temperature=1.):
    *_, n, dim = x.shape
    ...
    topks = jt.concat(topk_tensors, dim=-1)
    return topks
...
class RankNet(nn.Module):
    ...
    def forward(self, q, k, topk=1):
        ...
        # TODO: 是否需要进一步修改？
        Corr = jt.linalg.einsum('bei,bej->bij', sq, sk)
...
def unfold_bucket(patch_size, input):
    ...
    output = nn.unfold(input, kernel_size=patch_w, stride=stride) #_, 256*2*2, 64 * 64
...
def fold_bucket(input):
    ...
    stride = patch_w
    output = nn.fold(input, output_size=(w, w), kernel_size=(patch_w, patch_w), stride=stride)
...
def align_feature(R, dots, v):
    ...
    b_v_r = jt.linalg.einsum('buvk,bvtd->butkd', R, b_v)

    b_v_r = jt.reshape(b_v_r, (b, bucket_num, -1, D))
    b_v = b_v_r  # [2, 4096, 12, 3]     dots: [2, 4096, 4, 12]
    # out = b_v[:, :, 0, :]

    out = jt.linalg.einsum('buij,buje->buie', dots, b_v)
    out = fold_bucket(out)
    out = jt.reshape(out, (b, N, D))
...
def align_feature_v(R, dots, v):
    ...
    b_v_l = jt.linalg.einsum('buikj,bujd->buikd', dots, b_v)  # 2, 4096, 4, 3, 256
    b_v_r = jt.linalg.einsum('buvk,bvikd->buid', R, b_v_l)  # 2, 4096, 4, 256
...
class RAS(nn.Module):
    ...
    def execute(self, q, k, R):
        ...
        b_k_r = jt.linalg.einsum('buvk,bvtd->butkd', R, b_k)  #  2, 4096, 4, 3, 256
        b_k_r = jt.reshape(b_k_r, (b, buckets, self.patch_size, self.n_top, D))

        dots_ = jt.linalg.einsum('buid,bujkd->buijk', b_q, b_k_r)
