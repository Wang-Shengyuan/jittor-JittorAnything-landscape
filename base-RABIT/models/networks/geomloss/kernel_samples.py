...
def kernel_tensorized(α, x, β, y, blur=.05, kernel=None, name=None, potentials=False,**kwargs):
    ...
    a_x = jt.matmul( K_xx, α.detach().unsqueeze(-1) ).squeeze(-1)  # (B,N,N) @ (B,N) = (B,N) 
    b_y = jt.matmul( K_yy, β.detach().unsqueeze(-1) ).squeeze(-1)  # (B,M,M) @ (B,M) = (B,M)
    b_x = jt.matmul( K_xy, β.unsqueeze(-1)          ).squeeze(-1)  # (B,N,M) @ (B,M) = (B,N) 

    
    if potentials:
        a_y = jt.matmul( K_xy.transpose(1,2), α.unsqueeze(-1)).squeeze(-1)  # (B,M,N) @ (B,N) = (B,M)
...
def max_diameter(x, y):
    mins = jt.misc.stack((x.min(dim=0)[0], y.min(dim=0)[0])).min(dim=0)[0]
    maxs = jt.misc.stack((x.max(dim=0)[0], y.max(dim=0)[0])).max(dim=0)[0]
...
def kernel_multiscale(α, x, β, y, blur=.05, kernel=None, name=None, 
    ...
    with jt.no_grad():  # Compute our block-sparse reduction ranges:
