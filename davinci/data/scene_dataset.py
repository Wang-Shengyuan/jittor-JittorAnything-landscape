...
class SceneDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        if opt.phase != "test":
            image_paths = sorted(make_dataset(root + "imgs", recursive=True, read_cache=cache, write_cache=False))
            label_paths = sorted(make_dataset(root + "labels", recursive=True, read_cache=cache, write_cache=False))
        else:
            image_paths = sorted(
